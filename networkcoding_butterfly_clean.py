...
class network_coding_butterfly(object):
    ...
    def __keybid_to_bid(self, kbid):
        """
        INFO: unused method! Fill depending on the configuration of each testbed

        Returns the BID in tuple format from a key BID string
        """
    ...
    def __get_dst_bids(self, sbid, flow):
        ...
        #pbid = sbid     # Potential BID #XXX This must be a tuple

        l = flow["l"]
        dst_bid__l = sbid[l] + flow["x"][l]
        print "\n\nsbid before increment: %s\n\n" % str(sbid)
        print "\n\ndst_bid after increment: %s\n\n" % str(dst_bid__l)
        dbids = self.__find_dest_bids(dst_bid__l, l)
        return dbids

        #XXX XXX Delete
        """
        if flow["id"] == self.a_flow["id"]:
    ...
    def __add_flow_entries(self, src_dpid, in_port, out_ports, vlan_id, event):
        idle_timeout = 5
        hard_timeout = 0#10 - Avoids unnecessary messages
...
#        ####END DEBUG INFO
    ...
    def __route_packet(self, src_dpid, in_port, vlan_id, mpls_tag, event):
        ...
        try:
            ...
            src_bid = self.__dpid_to_bid(src_dpid, in_port) #XXX Must Be a TUPLE
            flow = self.__vlan_to_flow(vlan_id) # XXX Must be a DICT
